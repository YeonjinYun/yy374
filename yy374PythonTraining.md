...
def value(x):
  if x==10:
    ...
    print('The value is not 1 nor 10.')
x=3
print(value(x))
```
...
2. Write a `for` loop that takes a variable with an initial value of 0, and adds the current index to the previous value of that variable (i.e. your variable should grow in size every iteration). Perform the iteration 20 times, and have the final value be printed at the end.

```python
value=0
for i in range(0,20):
  value=value+i
print('The final value after 20 times iteration is',value,'.')
```
...
u.set_sig_figs(2:int)
valsin4=np.sin(4)*u.m
valsin4
```
...
mylist=[1,2,3,4,5]
len(mylist)
myarray=np.asarray(mylist*u.m)
myarray55=np.zeros((5,5))
myarray55
